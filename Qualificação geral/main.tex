...
% citestyle=numeric-comp ativa a compressão automática de citações consecutivas
\usepackage[backend=biber,style=abnt-numeric,citestyle=numeric-comp,sorting=none,ittitles,uniquename=init,giveninits,justify,indent,maxbibnames=10,minbibnames=6]{biblatex}
\addbibresource{references.bib}

% Desabilitar substituição de autores repetidos por traços
\renewcommand*{\bibnamedash}{\printnames{author}}
...
% - citestyle=numeric-comp já ativa compressão automática
% - 3 ou mais referências consecutivas: [1-5]
% - 2 referências ou menos: [1, 2]

% Configurar limite mínimo para compressão (padrão é 3)
...
\incluircapafalse                  % Capa com logos
...
\incluirabstractfalse               % Abstract em inglês
...
\incluirconclusaotrue              % Capítulo: Resultados esperados
...
\input{sections/0_pre_textuais.tex}
...
\ifincluirintroducao
\input{sections/1_introducao.tex}
\fi
\ifincluirobjetivos
\input{sections/2_objetivos.tex}
\fi
\ifincluirmetodologia
\input{sections/3_metodologia.tex}
\fi
\ifincluirconclusao
\input{sections/4_resultados_esperados.tex}
\fi
